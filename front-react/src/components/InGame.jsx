...
export default function InGame() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      await fetchStocks();
      await GetBalance();
      await updateTradeInfo(stockName);
    } catch (error) {
  ...
  const updateTradeInfo = async (name) => {
    try {
      const trade_stocks = await get_trade_stocks(name);
      setTradeQuantity(trade_stocks.stock_quantity); 
  ...
  const handleStockClick = async (selectedStockName, selectedStockPrice) => {
    ...
    await updateTradeInfo(selectedStockName);
